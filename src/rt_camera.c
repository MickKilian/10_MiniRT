...
/*   Updated: 2024/03/23 04:34:22 by mbourgeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cam_viewport_compute(t_rt *rt)
{
	rt->cam.viewport_w = 2.0 * tan(deg2rad(rt->cam.hfov) / 2.0)
		* rt->cam.focus_dist;
	rt->cam.viewport_h = rt->cam.viewport_w * ((double)rt->img_h
			/ (double)rt->img_w);
	...
	rt->cam.pixel_delta_u = vec3_scale(1.0 / rt->img_w, rt->cam.viewport_u);
	rt->cam.pixel_delta_v = vec3_scale(1.0 / rt->img_h, rt->cam.viewport_v);
